...
export class ScriptEditor implements OnInit {
  ...
  readonly trigger = computed<ScriptTrigger>(() => {
    const value = this.values()['trigger'];
    return value === 'before' || value === 'after' ? value : 'agent';
  });
  ...
  ngOnInit(): void {
    ...
    if (!['agent', 'before', 'after'].includes(String(values['trigger']))) patch['trigger'] = 'agent';
